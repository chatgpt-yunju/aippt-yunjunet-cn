...
// PDF to PNG images using pdf-poppler (supports multiple pages)
async function pdfToImages(pdfPath, outputDir) {
  ...
  await pdfPoppler.default.convert(pdfPath, opts)
  ...
  // Rename to consistent format: slide-01.png, slide-02.png
  const renamed = []
  for (let i = 0; i < files.length; i++) {
    const newName = path.join(outputDir, `slide-${String(i + 1).padStart(2, '0')}.png`)
    if (files[i] !== newName) {
  ...
  console.log(`[PDF转图片] 成功转换 ${renamed.length} 页`)
  return renamed
}
...
只输出讲解词，不需要其他格式。`
  ...
  try {
    ...
    // 使用简单的fallback，避免编码问题
    if (pageNum === 1) {
      return `大家好，欢迎观看本节内容。这一页主要介绍了相关内容要点。`
    } else if (pageNum === totalPages) {
      return `以上是本节内容的全部讲解，感谢观看。`
    } else {
      return `接下来我们看第${pageNum}页的内容。`
    }
  }
}
...
async function callAI(prompt, userId = 1) {
  ...
  try {
    ...
    console.log('[AI] Using fallback mock response')
    return generateMockResponse(prompt)
  }
}

// Mock AI response for demo when gateway is unavailable
function generateMockResponse(prompt) {
  // Extract slide info from prompt if possible
  const pageMatch = prompt.match(/第\s*(\d+)\s*页?.*?共\s*(\d+)\s*页?/i)
  const pageNum = pageMatch ? pageMatch[1] : '1'
  const totalPages = pageMatch ? pageMatch[2] : '5'

  const templates = [
    `这一页主要介绍了本次演示的核心内容。通过详细的讲解，我们可以更好地理解该主题的重要性和应用场景。`,
    `本页内容涵盖了关键概念和知识点。让我们深入分析其中的要点，并结合实际案例进行说明。`,
    `这个部分重点讲述了方法论和实践技巧。通过以下几个方面的阐述，帮助大家掌握核心技能。`,
    `本页总结了前面的内容，并提出了下一步的建议和展望。希望大家能够学以致用，取得更好的效果。`,
    `最后，让我们回顾一下今天分享的核心要点。如果有任何问题，欢迎随时交流讨论。`
  ]

  const idx = (parseInt(pageNum) - 1) % templates.length
  return `【第${pageNum}页讲解】${templates[idx]}（共${totalPages}页）`
}

// Text to speech using edge-tts CLI with proper encoding
async function textToSpeech(text, outputPath, voice = 'zh-CN-XiaoxiaoNeural') {
  return new Promise((resolve, reject) => {
    // Write text to temp file to avoid encoding issues with command line
    const tmpFile = outputPath.replace('.mp3', '.txt')
    fs.writeFileSync(tmpFile, text, 'utf8')

    const cmd = `edge-tts --voice ${voice} --file "${tmpFile}" --write-media "${outputPath}"`
    console.log(`[TTS] Running: ${cmd}`)

    exec(cmd, { timeout: 60000, encoding: 'utf8' }, (err, stdout, stderr) => {
      // Clean up temp file
      ...
      if (err) {
        ...
        console.log(`[TTS] Success: ${outputPath}`)
      }
      resolve()
    })
  })
